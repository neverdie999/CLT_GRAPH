...
class MainMgmt {
  ...
  createVertex(opt) {
    const vertex = this.vertex.createVertex(opt);
  }
  ...
  reSizeBoundaryAsObjectDragged(infos) {
    ...
    let svg = this.calculateSvgAreaWithId(infos);
   ...
   */
  resetSizeBoundary() {
    this.resetSizeBoundaryOneArea('#svg1');
    this.resetSizeBoundaryOneArea('#svg2');
    this.resetSizeBoundaryOneArea('#svg3');
  }

  resetSizeBoundaryOneArea(id) {
    d3.select(id).selectAll(`.${HTML_BOUNDARY_CONTAINER_CLASS}`).each((d, i, node) => {
      let orderObject = 0;
  ...
  checkDragObjectInsideBoundary(srcInfos, type) {
    ...
    let reverseBoundary = null;
    if (type === 'B') {
      reverseBoundary = reverse(this.objectUtils.getDataContainerFromBoundaryId(srcInfos.id).boundary);
    }
    else {
      reverseBoundary = reverse(this.objectUtils.getDataContainerFromVdertexId(srcInfos.id).boundary);
    }
  ...
  calculateSvgArea(options) {
    ...
    // dataContainer = this.dataContainer1;
      return {svg, dataContainer: this.dataContainer1};

    if (svg === 'svg2')
      return {svg, dataContainer: this.dataContainer2};
    // dataContainer = this.dataContainer2;
    if (svg === 'svg3')
      return {svg, dataContainer: this.dataContainer3};
    // dataContainer = this.dataContainer3;
    // return {svg, dataContainer, x, y};
  }

  calculateSvgAreaWithId(options) {
    ...
    if (bound1.length != 0) return '#svg1';
    if (bound2.length != 0) return '#svg2';
    if (bound3.length != 0) return '#svg3';
  }
  ...
  clearAll(options) {
    ...
    let {svg, dataContainer} = this.calculateSvgArea(options);
    ...
    if (options != null) {
      ...
      // Clear all data cotainer for vertex, boundary, edge
      this.dataContainer1.vertex = [];
      this.dataContainer1.boundary = [];
      this.dataContainer1.edge = [];
      this.dataContainer2.vertex = [];
      this.dataContainer2.boundary = [];
      this.dataContainer2.edge = [];
      this.dataContainer3.vertex = [];
      this.dataContainer3.boundary = [];
      this.dataContainer3.edge = [];
      this.initMarkerArrow();
      this.initPathConnect();
      this.initEdgePath();
      this.initBBoxGroup();
   ...
   * Update size
   */
  updateAttrBBoxGroup(data, area) {
    d3.select(area).select('#dummyBBox').attr('x', data.x);
    d3.select(area).select('#dummyBBox').attr('y', data.y);
    d3.select(area).select('#dummyBBox').attr('width', data.type == "B" ? data.width - 20 : data.width - 7);
    d3.select(area).select('#dummyBBox').attr('height', data.type == "B" ? data.height : data.height - 3);
    d3.select(area).select('#dummyBBox').style("display", "block");
    d3.select(area).select(d3.select("#dummyBBox").node().parentNode).moveToFront();
  }
  ...
  /**
   * Listen on container of an SVG scroll
   */
  initListenerContainerSvgScroll() {
    $('#container-input-message, #container-operation, #container-output-message').on('scroll', (e) => {
      let svg = $(e.target).attr("ref");
      this.onContainerSvgScroll(svg);
    });
  }

  onContainerSvgScroll(svg) {
    // Find the vertices exit edge on current SVG
    let edgeInViews = this.objectUtils.getVerticesHasConnectInSvg(svg);
    let length = edgeInViews.length;
    for (let i = 0; i < length; i++) {
      let vertexId = edgeInViews[i];
      this.vertex.updatePathConnect(vertexId);
    }
  }
};
