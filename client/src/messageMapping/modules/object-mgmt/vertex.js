...
class Vertex {
  ...
  createVertex(options) {
    ...
    let {x, y, name, description, data, id, parent, mandatory, repeat, isMenu, vertexType, isImport, connectType, groupType, svg} = options;
    // To do: Read or load from config.
    connectType = CONNECT_TYPE.BOTH;
    let {dataContainer} = this.mainMgmt.calculateSvgArea(options);
    if (!vertexType)
      return;
    ...
    dataContainer.vertex.push(vertexInfo);

    let group = d3.select(`#${svg}`).selectAll(`.${HTML_VERTEX_CONTAINER_CLASS}`)
      .data(dataContainer.vertex)
      .enter().append("g")
  ...
  dragged(self) {
    return (d) => {
      ...
      let {x, y} = self.setPositionJustInSvg(d3.event, d);
      d.x = x;
      d.y = y;
      // Transform group
  ...
  removeVertex(vertexId) {
    ...
    let dataContainerTmp = this.objectUtils.getDataContainerFromVdertexId(vertexId);
    let data = _.remove(dataContainerTmp.vertex, (e) => {
  ...
  /**
   * Calculate position of svg element relative to current view
   * @param id
   * @param prop
   * @param type
   * @param svg
   * @returns {{x: *, y: number}}
   */
  getCoordinateProperty(id, prop, type, svg) {
    if (!type)
    ...
    return {
      x: type === TYPE_POINT.OUTPUT ? axisX + VERTEX_ATTR_SIZE.GROUP_WIDTH + containerSvg.offset().left : axisX + containerSvg.offset().left,
      y: axisY - parentSvg.scrollTop()
    };
  }
  ...
  updatePathConnect(vertexId) {
    ...
    srcPaths.forEach(src => {
      let edgeId = src.id;
      let {prop, svg} = src.source;
      let {x, y} = this.getCoordinateProperty(vertexId, prop, TYPE_POINT.OUTPUT, svg);
      src.source.x = x;
      src.source.y = y;
    ...
    desPaths.forEach(des => {
      let edgeId = des.id;
      let {prop, svg} = des.target;
      let {x, y} = this.getCoordinateProperty(vertexId, prop, TYPE_POINT.INPUT, svg);
      des.target.x = x;
      des.target.y = y;
  ...
  startConnect(self) {
    return function () {
      ...
      let svg = d3.select(d3.event.sourceEvent.target).attr("svg");
      const source = self.getCoordinateProperty(sourceId, prop, TYPE_POINT.OUTPUT, svg);
  ...
  drawConnect() {
    return function () {
      if (COMMON_DATA.isCreatingEdge) {
        let x = d3.mouse(d3.select('#simulateConnect').node())[0];
        let y = d3.mouse(d3.select('#simulateConnect').node())[1];
  ...
  endConnect(self) {
    return function (d) {
      ...
      if (d3.event.sourceEvent.target.tagName == "circle" && this != d3.event.sourceEvent.target) {
        ...
        let prop = d3.select(d3.event.sourceEvent.target).attr("prop");
        let svg = d3.select(d3.event.sourceEvent.target).attr("svg");
        const target = self.getCoordinateProperty(targetId, prop, TYPE_POINT.INPUT, svg);
  ...
  /**
   * Calculate position relative with limit left and right
   * @param event
   * @param d
   * @returns {{x: number, y: number}}
   */
  setPositionJustInSvg(event, d) {
    // Limit left
    let x = event.x < DEFAULT_CONFIG_GRAPH.MIN_OFFSET_X ? DEFAULT_CONFIG_GRAPH.MIN_OFFSET_X : event.x;
    let y = event.y < DEFAULT_CONFIG_GRAPH.MIN_OFFSET_Y ? DEFAULT_CONFIG_GRAPH.MIN_OFFSET_Y : event.y;
    // limit right
    let limitWidth = $(`#${d.svg}`).width();
    let {width} = this.objectUtils.getBBoxObject(d.id);
